...
function GuardActivity() {//change here
    const { Bg, Loading } = useLoadAsset(CharacterActivityAsset.guard)//change here
    ...
    useEffect(() => {
        var sound = new Howl({
            src: [`/internal/audio/SB_26_Audio_39.mp3`],//change here
        });
        sound.play();
        sound.on('end', () => {
    ...
    useEffect(() => {
        if (characteractivity && Ref27.current && !Loading) {//change here
            try {
    ...
    const rightAnswerSound = () => {
        if (enableButton) {
            var sound = new Howl({
                src: [`/internal/audio/SB_26_Audio_41.mp3`],//change here
            });
            sound.play();
            sound.on('end', () => {
        ...
        <Scenes
            ...
            sprites={
                <>
                    ...
                    <div ref={Ref27} className="fireFighterSceneIcon" id="characterAnimation"></div>//change here
