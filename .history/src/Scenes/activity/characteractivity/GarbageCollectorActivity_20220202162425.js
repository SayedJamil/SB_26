...
function GarbageCollectorActivity() {//change here
    const { Bg, Loading } = useLoadAsset(CharacterActivityAsset.garbageCollector)//change here
    ...
    useEffect(() => {
        if (characteractivity && Ref25.current && !Loading) {//change here
            try {
    ...
    const rightAnswerSound = () => {
        if (enableButton) {
            var sound = new Howl({
                src: [`/internal/audio/SB_26_Audio_13.mp3`],//change here
            });
            sound.play();
            sound.on('end', () => {
        ...
        <Scenes
            ...
            sprites={
                <>
                    ...
                    <div ref={Ref25} className="doctorSceneIcon" id="characterAnimation"></div>//change here
