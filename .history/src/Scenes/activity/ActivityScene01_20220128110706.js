...
function ActivityExplainScene01() {
    ...
    useEffect(() => {
        if (activityscene01 && !Loading && !isLoading) {
            PlayAudio(activityscene01?.sounds[0])
        }
    }, [Assets, Loading, isLoading])

    useEffect(() => {
        if (activityscene01 && Ref1.current && !Loading) {
            try {
                lottie.loadAnimation({
                    ...
                    loop: true,
                    autoplay: true,
                    animationData: activityscene01?.lottie[0],
                })
            } catch (err) {
                console.log(err)
            }
        ...
        <Scenes
            ...
            sprites={
                <>
                    ...
                    <Image src={activityscene01?.sprites[0]} alt="txt" className="iconGirl" />
