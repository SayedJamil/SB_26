...
function TeacherActivity() {//change here
    ...
    useEffect(() => {
        var sound = new Howl({
            src: [`internal/audio/SB_26_Audio_21.mp3`],//change here
        });
        sound.play();
        sound.on('end', () => {
    ...
    const rightAnswerSound = () => {
        if (enableButton) {
            var sound = new Howl({
                src: [`internal/audio/SB_26_Audio_22.mp3`],//change here
            });
            sound.play();
            sound.on('end', () => {
        ...
        <Scenes
            ...
            sprites={
                <>
                    ...
                    <Image src={teacherScene?.sprites[toolNum]} alt="txt" className={`${position ? "bottomEquipButton" : "topEquipButton"}`} onClick={() => rightAnswerSound()} /> //change here
                    {(correct) ? <Image src={teacherScene?.sprites[18]} alt="txt" className={`${position ? "bottomHighlightIcon" : "topHighlightIcon"}`} onClick={() => rightAnswerSound()} /> : null}//change here
